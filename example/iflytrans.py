    ...
    def gene_request(self, apiname, data, files=None, headers=None):
        ...
        if result["ok"] == 0:
            ...
            if apiname == api_get_result:
                global user_text
                if result['data']!='[]':
                    user_text = json.loads(result['data'])[-1]['onebest']
                    print('------------')
                    print(user_text)
                    print('----------------')
                else:
...
def get_usertext():
    api = RequestApi(appid="5e61cede", secret_key="411298a287276d56dcc14854dae7cb85", upload_file_path="./assets/usersay.wav")
    api.all_api_request()
    print(user_text)
    return user_text
